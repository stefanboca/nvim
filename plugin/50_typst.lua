local map = _G.Config.nmapb_lleader
...
local function map_start(client, bufnr)
  map("p", function() _G.Tinymist.preview_start(client, bufnr) end, "Typst start preview", bufnr)
end
local function on_start(client, bufnr, result)
  unmap("p", bufnr)

  map("p", function() _G.Tinymist.preview_open(bufnr) end, "Typst open preview", bufnr)
  map("S", function() _G.Tinymist.preview_stop(client, bufnr) end, "Typst stop preview", bufnr)
  map("s", function() _G.Tinymist.toggle_autoscroll(bufnr) end, "Autoscroll toggle", bufnr)
  ...
  vim.api.nvim_create_autocmd({ "CursorMoved", "CursorMovedI" }, {
    ...
    buffer = bufnr,
    callback = function()
      if not (_G.Tinymist.open_previews[bufnr] and _G.Tinymist.open_previews[bufnr].scroll_enabled) then return end
  ...
  vim.ui.open(uri)
  _G.Tinymist.open_previews[bufnr] = {
    uri = uri,
    scroll_enabled = true,
    client_id = client.id,
  }
...
function _G.Tinymist.preview_start(client, bufnr)
  if _G.Tinymist.open_previews[bufnr] then return end
...
function _G.Tinymist.preview_open(bufnr)
  if not _G.Tinymist.open_previews[bufnr] then return end

  vim.notify("Opening preview", vim.log.levels.INFO)
  vim.ui.open(_G.Tinymist.open_previews[bufnr].uri)
end
local function on_stop(client, bufnr)
  if not _G.Tinymist.open_previews[bufnr] then return end
  _G.Tinymist.open_previews[bufnr] = nil
...
function _G.Tinymist.toggle_autoscroll(bufnr)
  _G.Tinymist.open_previews[bufnr].scroll_enabled = not _G.Tinymist.open_previews[bufnr].scroll_enabled
  if _G.Tinymist.open_previews[bufnr].scroll_enabled then
    print("  autoscroll")
  else
    print("noautoscroll")
  end
